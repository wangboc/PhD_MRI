function [rough_map, regularization_image]=JSENSE_estimate_maps(full_kspace_data,center_lines,center_locs,poly_tick,center_data,Image_size, FEs,PEs);
%Swati Rane, TAMU
%September 1, 2004
%--------------------------------------------------
%---------------------------------------------------
...
k=C/2;
kc=center_lines/4; %center_lines/2
w=20; %20
epsilon=2;
k1=center_lines/4;%center_lines/2  (32-8-15) (24-8-11)  (16-8-7)
option=2;

%tukey_window = cosine_taper_window(k,kc,w,epsilon,k1,option);
tukey_window = cosine_taper_window(256,7,20,2,108,2); %32
%tukey_window = cosine_taper_window(128,15,20,2,108,2);  %64
...
% if(center_data~=0)
    [trashf trashp num_coils]=size(center_data);
    for(i=1:num_coils)
       k_coil(:,:,i)=zeros(Ro,C);
