function recon = senserecon_J(KspaceDataWeighted,coilprofile,reduc);
%
...
for s = 1 : coiln
    coilimagealias(:,:,s)=fftshift(ifft2(KspaceDataWeighted(:,:,s))).*alt1;
end
figure; imshow(rot90(abs(coilprofile(:,:,1)),-1),[]);
Img_sos=sqrt(sum([abs(coilimagealias)].^2,3));
figure; imshow(rot90(abs(Img_sos),-1),[]);
